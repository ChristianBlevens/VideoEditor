...
import { loadSubtitleSettingsForSelectedClip } from '../features/subtitles/subtitle-system.js';
import { renderClipProperties } from '../features/properties/clip-properties.js';
import { updateTranscriptForSelectedClip } from '../features/transcripts/transcript.js';
...
export function deleteClip(clipId) {
    ...
    const index = state.selection.selectedClips.indexOf(clipId);
    if (index !== -1) {
        state.selection.selectedClips.splice(index, 1);
    }
...
export function createTranscript(transcriptData) {
    ...
    // Auto-regenerate subtitles when new transcript is created
    if (state.playback.subtitlesEnabled) {
        import('../features/subtitles/subtitle-system.js').then(({ convertTranscriptsToSubtitles }) => {
...
export function updateTranscript(transcriptId, updates) {
    ...
    // Auto-regenerate subtitles when transcript changes
    if (state.playback.subtitlesEnabled) {
        import('../features/subtitles/subtitle-system.js').then(({ convertTranscriptsToSubtitles }) => {
            convertTranscriptsToSubtitles();
        });
...
export function deleteTranscript(transcriptId) {
    ...
    // Auto-regenerate subtitles when transcript is deleted
    if (state.playback.subtitlesEnabled) {
        import('../features/subtitles/subtitle-system.js').then(({ convertTranscriptsToSubtitles }) => {
            convertTranscriptsToSubtitles();
        });
...
/**
 * Select a clip (replaces current selection with this clip)
...
export function selectClip(clipId) {
    if (clipId === null) {
        ...
        state.selection.selectedClips = [clipId];
    }

    // Update panels for the newly selected clip
    loadSubtitleSettingsForSelectedClip();
    renderClipProperties();
    updateTranscriptForSelectedClip();

    // Update transcript content and pace analysis for selected clips
    import('../features/transcripts/transcript-core.js').then(({ renderCompiledTranscript }) => {
        renderCompiledTranscript();
    });
    import('../features/transcripts/pace-analyzer.js').then(({ showPaceAnalysis }) => {
        showPaceAnalysis();
    });
}

/**
 * Toggle clip selection (add or remove from selection)
...
export function toggleClipSelection(clipId) {
    ...
    if (index === -1) {
        // Not selected, add it
        state.selection.selectedClips.push(clipId);
    } else {
...
export function selectAllClips() {
    ...
    // Only select video and audio clips (not attached-audio)
    state.selection.selectedClips = allClips
        .filter(clip => clip.type !== 'attached-audio')
        .map(clip => clip.id);
 ...
 * Deselect all clips
 */
export function deselectAllClips() {
    state.selection.selectedClips = [];
...
/**
 * Get selected clip (returns first selected clip for backwards compatibility)
 * @returns {Object|null}
 */
export function getSelectedClip() {
    if (state.selection.selectedClips.length === 0) return null;
    return getClip(state.selection.selectedClips[0]);
}
...
export function getSelectedClips() {
    return state.selection.selectedClips
        .map(clipId => getClip(clipId))
