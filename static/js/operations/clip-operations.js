...
import { renderAllClips, renderClip } from '../rendering/rendering.js';
import { renderClipProperties } from '../features/properties/clip-properties.js';
import { pushToHistory } from './history-manager.js';
import { addTrackAction } from '../features/timeline/track-actions.js';
...
export async function splitClipAtPlayhead() {
    ...
    const clipMediaStart = clipToSplit.mediaStart || 0;
    const clipMediaEnd = clipToSplit.mediaEnd || clipToSplit.sourceDuration || clipToSplit.duration;
    ...
    const firstClipId = createClip({
        ...
        mediaStart: clipMediaStart,
        mediaEnd: clipMediaStart + firstClipDuration
    });
    ...
    const secondClipId = createClip({
        ...
        mediaStart: clipMediaStart + splitPoint,
        mediaEnd: clipMediaEnd
    });
    ...
    if (clipToSplit.type === 'video') {
        ...
        if (attachedAudio) {
            ...
            const audioMediaStart = attachedAudio.mediaStart || 0;
            const audioMediaEnd = attachedAudio.mediaEnd || attachedAudio.sourceDuration || attachedAudio.duration;
            ...
            createClip({
                ...
                mediaEnd: audioMediaStart + firstClipDuration
            });
            ...
            createClip({
                ...
                mediaStart: audioMediaStart + splitPoint,
                mediaEnd: audioMediaEnd
            });
    ...
    if (state.search.matches && state.search.matches.length > 0) {
        ...
        const { renderSearchMarkersOnTimeline } = await import('../features/transcripts/transcript.js');
    ...
    const { updateViewportRange, generateTimeMarkers, updatePlayheadPosition, updateInOutMarkers } = await import('../features/timeline/timeline.js');
    updateViewportRange();
    generateTimeMarkers();
...
function findClipAtTime(time) {
    ...
    // First try to get a selected clip if any of them are at this time
    if (state.selection.selectedClips.length > 0) {
        for (const clipId of state.selection.selectedClips) {
            const selectedClip = state.project.clips.get(clipId);
...
export async function deleteSelectedClip() {
    ...
    if (success) {
        ...
        const { resetTimelineBounds, updateViewportRange, generateTimeMarkers, updatePlayheadPosition, updateInOutMarkers, updateZoomLevel } = await import('../features/timeline/timeline.js');
        resetTimelineBounds();
        updateViewportRange();
        updateZoomLevel();
        generateTimeMarkers();
