...
function createSnapshot() {
    ...
    return {
        ...
        selection: {
            selectedClips: [...state.selection.selectedClips],  // Copy array
...
async function restoreSnapshot(snapshot) {
    ...
    // Restore selection
    state.selection.selectedClips = [...snapshot.selection.selectedClips];  // Copy array
    ...
    const { renderCompiledTranscript } = await import('../features/transcripts/transcript.js');
 ...
 */
export function pushToHistory(description = 'Edit', type = 'update') {
    const snapshot = createSnapshot();

    // Add metadata for history panel
    ...
    // Dispatch history changed event
...
function dispatchHistoryChanged() {
    document.dispatchEvent(new CustomEvent('history-changed', {
        detail: {
            history: state.history.undoStack,
            currentIndex: state.history.undoStack.length - 1
        }
    }));
}

/**
 * History manager interface for history panel compatibility
 */
export const historyManager = {
     ...
     */
    getCurrentIndex() {
        // Current state is after the last undo stack entry
        return state.history.undoStack.length - 1;
    },
