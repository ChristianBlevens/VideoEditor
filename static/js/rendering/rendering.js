...
import { getAllClips, groupClipsByTrack, getClipEdits } from '../operations/operations.js';
...
/**
 * Update video placeholder visibility based on whether clips exist
 */
export function updateVideoPlaceholder() {
    if (!elements.videoPlaceholder) return;

    // Check if there are any video clips on the timeline
    const hasVideoClips = Array.from(state.project.clips.values())
        .some(clip => clip.type === 'video');

    // Hide placeholder if video clips exist, show if no video clips
    elements.videoPlaceholder.style.display = hasVideoClips ? 'none' : 'flex';
}
...
async function rerenderAllActionIndicators() {
    const { renderAllTrackActions } = await import('../features/timeline/track-actions.js');
...
function createClipElement(clip) {
    ...
    // Add selection state
    if (state.selection.selectedClips.includes(clip.id)) {
...
function updateClipElement(element, clip) {
    ...
    element.classList.toggle('selected', state.selection.selectedClips.includes(clip.id));
...
export async function startClipRename(clipId, nameElement) {
    ...
    const saveName = () => {
        ...
        if (newName && newName !== currentName) {
            ...
            import('../features/properties/clip-properties.js').then(({ renderClipProperties }) => {
                renderClipProperties();
            });
            // Update transcript panel
            import('../features/transcripts/transcript.js').then(({ updateTranscriptForSelectedClip }) => {
                updateTranscriptForSelectedClip();
            });
            // Update subtitles panel
            import('../features/subtitles/subtitle-system.js').then(({ loadSubtitleSettingsForSelectedClip }) => {
        ...
        // Remove event listeners before removing element to prevent blur from firing
