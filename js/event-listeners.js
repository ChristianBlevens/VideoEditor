...
import { zoomTimeline, resetZoom, setInPoint, setOutPoint, updateInOutMarkers, panTimeline, updateAllTimelineElements, updatePlayheadPosition } from './timeline.js';
import { splitClipAtPlayhead, deleteSelectedClip, toggleClipMute } from './clip-operations.js';
import {
    transcribeSelectedClip,
    addFillerWordsToTargets,
    addProfanityToTargets,
    addPausesToTargets,
...
import { newProject, openProject, saveProject, undo, redo } from './project.js';
import {
    openExportModal,
    closeExportModal,
    startExport,
    ...
    handleMarkersToggle,
    handleTranscriptsToggle,
    handleTimelineScroll,
    showToast
} from './ui-utils.js';
import { handleMarkerTrackClick, closeMarkerModal, saveNewMarker } from './markers.js';
...
export function initializeEventListeners() {
    ...
    const showTranscriptsCheckbox = document.getElementById('show-transcripts');

    if (showMarkersCheckbox) {
        showMarkersCheckbox.addEventListener('change', handleMarkersToggle);
    }

    if (showTranscriptsCheckbox) {
        showTranscriptsCheckbox.addEventListener('change', handleTranscriptsToggle);
    }
    ...
    elements.findFillerBtn.addEventListener('click', addFillerWordsToTargets);
    elements.findProfanityBtn.addEventListener('click', addProfanityToTargets);
    elements.findPausesBtn.addEventListener('click', addPausesToTargets);
